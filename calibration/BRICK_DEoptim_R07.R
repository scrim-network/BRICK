...
##    => luse.doeclim, luse.gsic, luse.te, luse.simple, luse.dais
##
## Notes:
...
minimize_residuals_brick = function(
  ...
  if(!is.null(oidx$sl) & luse.brick[,"luse.gmsl"]) {
		sl.norm.resid = mean(abs( (obs$sl[oidx$sl]-brick.out$gmsl.out[midx$sl])/obs.err$sl[oidx$sl] ))
	}

  if(!is.null(oidx$gsic) & luse.brick[,"luse.gsic"]) {
		gsic.norm.resid = mean(abs( (obs$gsic[oidx$gsic]-brick.out$gsic.out$sle.gis[midx$gsic])/obs.err$gsic[oidx$gsic] ))
	}
