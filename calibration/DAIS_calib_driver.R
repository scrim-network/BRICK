...
obs.err <- 0.5*obs.err                       # assume all windows are 2*stdErr (last two actually are)
...
if(nnode.mcmc == 1) {
  t.beg <- proc.time()
  amcmc.out <- MCMC(log.post, niter.mcmc, parameters0.lhs, scale=step.mcmc, adapt=TRUE, acc.rate=accept.mcmc,
  ...
  t.beg <- proc.time()
  amcmc.out <- MCMC.parallel(log.post, niter.mcmc, parameters0.lhs, n.chain=nnode.mcmc, n.cpu=nnode.mcmc,
