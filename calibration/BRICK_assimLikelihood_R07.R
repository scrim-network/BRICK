##==============================================================================
##
##  -file = "BRICK_assimLikelihood_R07.R"   Origional code written July 2014
...
log.lik = function( parameters.in,
  ...
  if(!is.null(oidx$sl) & luse.brick[,"luse.gmsl"]) {

    # Grab the GMSL statistical parameters
  	sigma.gmsl=parameters.in[match("sigma.gmsl",parnames.in)]
  	rho.gmsl  =parameters.in[match("rho.gmsl"  ,parnames.in)]

    # Calculate the GMSL residuals; apply AR1 error model
    resid.sl= obs$sl[oidx$sl] - brick.out$gmsl.out[midx$sl]
    llik.sl = logl.ar1(resid.sl, sigma.gmsl, rho.gmsl, obs.err$sl[oidx$sl]) # AR(1)
