...
source('../calibration/BRICK_assimLikelihood_forward_DAIS_MWG.R')
source('../calibration/BRICK_assimLikelihood_forward_notDAIS_MWG.R')
...
##==========
##==========
## an actual Metropolis-within-Gibbs, between DAIS and non-DAIS

niter.batch <- 1e5
nbatch		<- 2
ind.dais.fixed <- c(19:31,40)
ind.notdais.fixed <- 1:length(parnames)
ind.notdais.fixed <- ind.notdais.fixed[-ind.dais.fixed]


# first batch
parameters.fixed <- p0.deoptim[ind.notdais.fixed]
parnames.fixed <- parnames[ind.notdais.fixed]
t.beg=proc.time()											# save timing (running millions of iterations so best to have SOME idea...)
amcmc.dais = MCMC(log.post_dais, niter.batch, p0.deoptim[ind.dais.fixed], scale=step.mcmc[ind.dais.fixed], adapt=TRUE, acc.rate=accept.mcmc,
					gamma=gamma.mcmc               , list=TRUE                  , n.start=round(0.01*niter.mcmc)    ,
					parnames.in=parnames[ind.dais.fixed], forcing.raw=forcing   , l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.notdais.fixed], bound.upper.fixed=bound.upper[ind.notdais.fixed],
					slope.Ta2Tg=slope.Ta2Tg		   , intercept.Ta2Tg=intercept.Ta2Tg, tstep=tstep					,
					ind.norm.data=ind.norm.data    , ind.norm.sl=ind.norm.sl    , mod.time=mod.time                 ,
					oidx = oidx.all                , midx = midx.all            , obs=obs.all                       ,
					obs.err = obs.err.all          , trends.te = trends.te      , trends.ais = trends.ais			,
					bound.lower=bound.lower[ind.dais.fixed], bound.upper=bound.upper[ind.dais.fixed], shape.invtau=shape.invtau      	,
					scale.invtau=scale.invtau      , rho.simple.fixed=rho.simple.fixed, sigma.simple.fixed=sigma.simple.fixed,
					luse.brick=luse.brick          , dais.prior.fit=dais.prior.fit, gsic.prior.fit=gsic.prior.fit   ,
					doeclim.prior.fit=doeclim.prior.fit, te.prior.fit=te.prior.fit, anto.prior.fit=anto.prior.fit   ,
					simple.prior.fit=simple.prior.fit)

parameters.fixed <- amcmc.dais$samples[niter.batch,]
parnames.fixed <- parnames[ind.dais.fixed]
amcmc.notdais = MCMC(log.post_nodais, niter.batch, p0.deoptim[ind.notdais.fixed], scale=step.mcmc[ind.notdais.fixed], adapt=TRUE, acc.rate=accept.mcmc,
					gamma=gamma.mcmc               , list=TRUE                  , n.start=round(0.01*niter.mcmc)    ,
					parnames.in=parnames[ind.notdais.fixed], forcing.raw=forcing, l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.dais.fixed], bound.upper.fixed=bound.upper[ind.dais.fixed],
					slope.Ta2Tg=slope.Ta2Tg		   , intercept.Ta2Tg=intercept.Ta2Tg, tstep=tstep					,
					ind.norm.data=ind.norm.data    , ind.norm.sl=ind.norm.sl    , mod.time=mod.time                 ,
					oidx = oidx.all                , midx = midx.all            , obs=obs.all                       ,
					obs.err = obs.err.all          , trends.te = trends.te      , trends.ais = trends.ais			,
					bound.lower=bound.lower[ind.notdais.fixed], bound.upper=bound.upper[ind.notdais.fixed], shape.invtau=shape.invtau      	,
					scale.invtau=scale.invtau      , rho.simple.fixed=rho.simple.fixed, sigma.simple.fixed=sigma.simple.fixed,
					luse.brick=luse.brick          , dais.prior.fit=dais.prior.fit, gsic.prior.fit=gsic.prior.fit   ,
					doeclim.prior.fit=doeclim.prior.fit, te.prior.fit=te.prior.fit, anto.prior.fit=anto.prior.fit   ,
					simple.prior.fit=simple.prior.fit)
...
for (ib in 2:nbatch) {
	...
	print(paste('Starting batch ',ib,'/',nbatch,'...',sep=''))
	# only DAIS
	parameters.fixed <- amcmc.notdais$samples[(ib-1)*niter.batch,]
	parnames.fixed <- parnames[ind.notdais.fixed]
	amcmc.extend = MCMC.add.samples(amcmc.dais, niter.batch,
					parnames.in=parnames[ind.dais.fixed], forcing.raw=forcing   , l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.notdais.fixed], bound.upper.fixed=bound.upper[ind.notdais.fixed],
					slope.Ta2Tg=slope.Ta2Tg		   , intercept.Ta2Tg=intercept.Ta2Tg, tstep=tstep					,
					...
					doeclim.prior.fit=doeclim.prior.fit, te.prior.fit=te.prior.fit, anto.prior.fit=anto.prior.fit   ,
					simple.prior.fit=simple.prior.fit)
	amcmc.dais <- amcmc.extend

	# sample from llik with only DAIS
	parameters.fixed <- amcmc.dais$samples[ib*niter.batch,]
	parnames.fixed <- parnames[ind.dais.fixed]
	amcmc.extend = MCMC.add.samples(amcmc.notdais, niter.batch,
					parnames.in=parnames[ind.notdais.fixed], forcing.raw=forcing, l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.dais.fixed], bound.upper.fixed=bound.upper[ind.dais.fixed],
					slope.Ta2Tg=slope.Ta2Tg		   , intercept.Ta2Tg=intercept.Ta2Tg, tstep=tstep					,
					...
					doeclim.prior.fit=doeclim.prior.fit, te.prior.fit=te.prior.fit, anto.prior.fit=anto.prior.fit   ,
					simple.prior.fit=simple.prior.fit)
	amcmc.notdais <- amcmc.extend
}

# one last DAIS batch, really long, to get them to converge
niter.final <- 1e5
print(paste('Starting final long DAIS batch...',sep=''))
parameters.fixed <- amcmc.notdais$samples[ib*niter.batch,]
parnames.fixed <- parnames[ind.notdais.fixed]
amcmc.extend = MCMC.add.samples(amcmc.dais, niter.final,
				parnames.in=parnames[ind.dais.fixed], forcing.raw=forcing   , l.project=l.project          		,
				parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
				bound.lower.fixed=bound.lower[ind.notdais.fixed], bound.upper.fixed=bound.upper[ind.notdais.fixed],
				slope.Ta2Tg=slope.Ta2Tg		   , intercept.Ta2Tg=intercept.Ta2Tg, tstep=tstep					,
...
##==========
##==========
## Just one blast through each? DAIS, then not-DAIS
...
parnames.fixed <- parnames[ind.notdais.fixed]
t.beg=proc.time()											# save timing (running millions of iterations so best to have SOME idea...)
amcmc.dais.once = MCMC(log.post_dais, niter.batch, p0.deoptim[ind.dais.fixed], scale=step.mcmc[ind.dais.fixed], adapt=TRUE, acc.rate=accept.mcmc,
					gamma=gamma.mcmc               , list=TRUE                  , n.start=round(0.01*niter.mcmc)    ,
					parnames.in=parnames[ind.dais.fixed], forcing.raw=forcing   , l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.notdais.fixed], bound.upper.fixed=bound.upper[ind.notdais.fixed],
...
parameters.fixed <- amcmc.dais.once$samples[niter.batch,]
parnames.fixed <- parnames[ind.dais.fixed]
amcmc.notdais.once = MCMC(log.post_nodais, niter.batch, p0.deoptim[ind.notdais.fixed], scale=step.mcmc[ind.notdais.fixed], adapt=TRUE, acc.rate=accept.mcmc,
					gamma=gamma.mcmc               , list=TRUE                  , n.start=round(0.01*niter.mcmc)    ,
					parnames.in=parnames[ind.notdais.fixed], forcing.raw=forcing, l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.dais.fixed], bound.upper.fixed=bound.upper[ind.dais.fixed],
...
##==========
##==========
## Just one blast through each?
## -> DAIS parameters each separately, (then all together?), then not-DAIS
...
# first batch through each DAIS parameter
parameters.fixed <- p0.deoptim[ind.notdais.fixed]
parnames.fixed <- parnames[ind.notdais.fixed]
for (ip in 1:ind.dais.fixed) {
	t.beg=proc.time()											# save timing (running millions of iterations so best to have SOME idea...)
	amcmc.dais.new = MCMC(log.post_dais, niter.batch, p0.deoptim[ind.dais.fixed], scale=step.mcmc[ind.dais.fixed], adapt=TRUE, acc.rate=accept.mcmc,
					gamma=gamma.mcmc               , list=TRUE                  , n.start=round(0.01*niter.mcmc)    ,
					parnames.in=parnames[ind.dais.fixed], forcing.raw=forcing   , l.project=l.project          		,
					parameters.fixed=parameters.fixed, parnames.fixed=parnames.fixed,
					bound.lower.fixed=bound.lower[ind.notdais.fixed], bound.upper.fixed=bound.upper[ind.notdais.fixed],
