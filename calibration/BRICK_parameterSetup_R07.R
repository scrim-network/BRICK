...
if (luse.gmsl) {
	parnames.gmsl   =c("a.gmsl","Teq.gmsl","sigma.gmsl"	,"rho.gmsl"	)        # parameters names
	p0.gmsl		      =c(0.0034  ,-0.3   		, .01      		, 0.5   		)        # initial parameter guesses
	bound.lower.gmsl=c(0.0   	 ,-1.5   		, 0          	, 0					)        # prior range lower bounds
	bound.upper.gmsl=c(0.0035	 , 1.5	 		, .05         , 0.999			)        # prior range upper bounds
	step.mcmc.gmsl	=0.05*(bound.upper.gmsl-bound.lower.gmsl) # set size for parameters in MCMC (proposals)
	index.model.gmsl=c(1,2)			# which are model parameters? (index within parnames.gmsl)
}
...
index.model = c(match(parnames.doeclim[index.model.doeclim],parnames),
								...
								match(parnames.gmsl[index.model.gmsl]      ,parnames))
