...
## edit the script 'BRICK_parameterSetup_SIMPLE-GSIC.R' before sourcing it.
##
...
require('DEoptim')
library(DEoptim)
source('../calibration/BRICK_DEoptim_SIMPLE-GSIC.R')
...
lines(obs.gsic.time, brick.out$gsic.out$sle.gis[midx.gsic], col="blue", lwd=2)
	# plot 4 -- GIS match
...
source('../calibration/BRICK_assimLikelihood_SIMPLE-GSIC.R')
...
## Determine when (in increments of 10s of 1000s of iterations, using Gelman and
## Rubin diagnostic) the parallel chains, which are assumed to be
## chain1=amcmc.par1[[1]]$samples and chain2=amcmc.par1[[2]]$samples and ...
...
##	Wong et al 2016 -- n.min = 3e5, but discard entire first half of chain
n.burnin = 5e5
...
## Write the calibrated parameters to a netCDF file
...
today=Sys.Date(); today=format(today,format="%d%b%Y")
filename.parameters = paste('../output_calibration/BRICK-model_calibratedParameters_gsic-simple_',today,'.nc',sep="")
