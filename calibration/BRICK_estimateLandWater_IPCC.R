...
itmp <- which(lw.time.1900==1961):which(lw.time.1900==1989)
lw.1900.norm <- lw.1900-mean(lw.1900[itmp])
lw.err.1900 <- (0.25*(-0.06--0.16)/1000)*sqrt(abs(lw.time.1900 - mean(1961:1990)))

#1971-2010: 0.12 [0.03 to 0.22]
lw.time.1970 <- 1970:2009
lw.time.1970.long <- 1960:2009
i1970 <- which(mod.time==lw.time.1970[1]):which(mod.time==lw.time.1970[length(lw.time.1970)])
i1970.long <- which(mod.time==lw.time.1970.long[1]):which(mod.time==lw.time.1970.long[length(lw.time.1970.long)])
lw.1970 <- (0.12/1000)*(lw.time.1970.long - lw.time.1970.long[1])
itmp <- which(lw.time.1970.long==1961):which(lw.time.1970.long==1990)
lw.1970.norm <- lw.1970-mean(lw.1970[itmp])
lw.1970.norm <- lw.1970.norm[which(lw.time.1970.long==lw.time.1970[1]):which(lw.time.1970.long==lw.time.1970[length(lw.time.1970)])]
lw.err.1970 <- (0.25*(0.2-0.03)/1000)*sqrt(abs(lw.time.1970 - mean(1961:1990)))

#1993-2010: 0.38 [0.26 to 0.49]
lw.time.1992 <- 1992:2009
lw.time.1992.long <- 1960:2009
i1992 <- which(mod.time==lw.time.1992[1]):which(mod.time==lw.time.1992[length(lw.time.1992)])
i1992.long <- which(mod.time==lw.time.1992.long[1]):which(mod.time==lw.time.1992.long[length(lw.time.1992.long)])
lw.1992 <- (0.38/1000)*(lw.time.1992.long - lw.time.1992.long[1])
itmp <- which(lw.time.1992.long==1961):which(lw.time.1992.long==1990)
lw.1992.norm <- lw.1992-mean(lw.1992[itmp])
lw.1992.norm <- lw.1992.norm[which(lw.time.1992.long==lw.time.1992[1]):which(lw.time.1992.long==lw.time.1992[length(lw.time.1992)])]
lw.err.1992 <- (0.25*(0.49-0.26)/1000)*sqrt(abs(lw.time.1992 - mean(1961:1990)))
