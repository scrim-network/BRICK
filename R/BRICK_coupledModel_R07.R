...
##							(here, we use luse.gmsl, for demonstration using Rahmstorf 2007
##							model for GMSL)
##
...
brick_model = function(
  ...
  if (luse.brick[,"luse.doeclim"]) {
	...
	#=============================================================================
	# GMSL - Rahmstorf model (2007)

	if (luse.brick[,"luse.gmsl"]) {

	  ## Grab GMSL parameters
		a.gmsl  =parameters.in[match("a.gmsl"		,parnames.in)]
		Teq.gmsl=parameters.in[match("Teq.gmsl"	,parnames.in)]

    ## Run gmsl_r07 model
    gmsl.out = gmsl_r07(a=a.gmsl, Teq=Teq.gmsl, Tg=temp.couple)

		## normalize
		itmp = ind.norm.data[match("sl",ind.norm.data[,1]),2]:ind.norm.data[match("sl",ind.norm.data[,1]),3]
		gmsl.out = gmsl.out - mean(gmsl.out[itmp])

		## Add this contribution to the total sea level rise
		slr.out = slr.out + gmsl.out

		brick.out[[outcnt]] = gmsl.out; names(brick.out)[outcnt]="gmsl.out"; outcnt=outcnt+1;
  ...
  if (luse.brick[,"luse.gsic"]) {
	  ...
	  ## Grab SIMPLE-GSIC parameters
		a.gsic    =parameters.in[match("a.gsic"    ,parnames.in)]
		b.gsic    =parameters.in[match("b.gsic"    ,parnames.in)]
		alpha.gsic=parameters.in[match("alpha.gsic",parnames.in)]
		beta.gsic =parameters.in[match("beta.gsic" ,parnames.in)]
		V0.gsic   =parameters.in[match("V0.gsic"   ,parnames.in)]

    ## Run SIMPLE (GSIC version model)
    gsic.out = simpleF(a=a.gsic, b=b.gsic, alpha=alpha.gsic,
											 beta=beta.gsic, V0=V0.gsic, Tg=temp.couple)

		## Add this contribution to the total sea level rise
		slr.out = slr.out + (gsic.out$sle.gis - mean(gsic.out$sle.gis[ind.norm.sl]))
		...
		gsic.out$sle.gis = gsic.out$sle.gis - mean(gsic.out$sle.gis[itmp])
    ...
    ## Run BRICK-TE (thermal expansion) model, using temp output from DOECLIM
  ...
  if (luse.brick[,"luse.dais"]) {
		...
		if(l.fprint) {
			...
			dSL.gsic= diff(gsic.out$sle.gis)
			dSL.te	= diff(te.out)
...
#			dais.out = daisanto(
	...
	itmp = ind.norm.data[match("sl",ind.norm.data[,1]),2]:ind.norm.data[match("sl",ind.norm.data[,1]),3]
	brick.out[[sum(luse.brick)+1]] = slr.out - mean(slr.out[itmp])
