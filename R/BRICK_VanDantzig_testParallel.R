...
# - ss.gev        time series of GEV parameters for storm surge (optional, but encouraged)
...
## Clip the storm surge parameters to the given time frame, if there are any
ss.gev = ss.gev[match(currentyear, time_frame):match(endyear, time_frame),]
...
if(is.null(surge.rise)) {
  vanDantzig.out = VD_NOLA_R(params=params.vd[1,], I_table=I_table, T=T, X=X,
    ...
    slope.h2i=slope.h2i, ss.gev=ss.gev, H0=H0)
} else {
  vanDantzig.out = VD_NOLA_R(params=params.vd[1,], I_table=I_table, T=T, X=X,
    local_sea_level=local_sea_level[,1], intercept.h2i=intercept.h2i,
    slope.h2i=slope.h2i, ss.gev=ss.gev, surge.rise=surge.rise[,1], H0=H0)
}
...
source('../Useful/MultipleOutput.R') # useful

library(doSNOW)
n.threads <- 2
cl <- makeCluster(n.threads)
registerDoSNOW(cl)
vandantzig_iteration <- function(params.vd.in, I_table, T, X, local_sea_level.in, intercept.h2i, slope.h2i, ss.gev, surge.rise=NULL, H0) {


#todo here now! adding input, running loop in parallel.


  # these are what take a long time.
  c(vanDantzig.ensemble[[1]][,i],
    vanDantzig.ensemble[[2]][,i],
    vanDantzig.ensemble[[3]][,i],
    vanDantzig.ensemble[[4]][,i],
    vanDantzig.ensemble[[5]][,i],
    vanDantzig.ensemble[[6]][,i],
    vanDantzig.ensemble[[7]][,i]) := VD_NOLA_R(params=params.vd.in, I_table=I_table,
                                              T=T, X=X, local_sea_level=local_sea_level.in,
                                              intercept.h2i=intercept.h2i, slope.h2i=slope.h2i,
                                              ss.gev=ss.gev, H0=H0)
...
#      slope.h2i=slope.h2i, ss.gev=ss.gev, H0=H0)
#  # these do not. and can probably be achieved with an lapply
...
result <- foreach(i=1:n.ensemble) %dopar% {
  # these are what take a long time.
  c(vanDantzig.ensemble[[1]][,i],
    vanDantzig.ensemble[[2]][,i],
    vanDantzig.ensemble[[3]][,i],
...
if(is.null(surge.rise)) {
  ...
  for (i in 1:n.ensemble) {
    vanDantzig.out = VD_NOLA_R(params=params.vd[i,], I_table=I_table, T=T, X=X,
      local_sea_level=local_sea_level[,i], intercept.h2i=intercept.h2i,
      slope.h2i=slope.h2i, ss.gev=ss.gev, surge.rise=surge.rise[,i], H0=H0)
    for (j in 1:ncol(vanDantzig.out)) {
		  vanDantzig.ensemble[[j]][,i] = vanDantzig.out[,j]
	  }
    setTxtProgressBar(pb, i)
